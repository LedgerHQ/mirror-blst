...
The `std` feature is enabled by default. Disable default features to build the
Rust wrapper as `no_std` on a hosted target. Targets without a standard library
continue to select `no_std` automatically.
